...
class LotusWorker:
    ...
    async def run(self):
        ...
        self.log_method(f'Starting Worker [{self.worker_id}]: Flush At: {self.flush_at}, Flush Interval: {self.flush_interval}')
        self.running = True
        while self.running:
            await self.upload()
    ...
    async def upload(self):
        ...
        try:
            ...
            self.log_method(f'Worker [{self.worker_id}] Acknowledged {len(batch)} items')
            for _ in batch:
